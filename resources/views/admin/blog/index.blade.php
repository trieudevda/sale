...
@section('content')
  <div class="flex items-center gap-3">
    <h3 class="mb-2 text-4xl font-bold tracking-tight text-heading md:text-5xl lg:text-6xl">{{__('admin.blog',['action' => __('admin.add')])}}</h3>
    <a type="button" href="{{ route('admin.blog.create') }}"
      class="text-white bg-brand box-border border border-transparent h-min hover:bg-brand-strong focus:ring-4 focus:ring-brand-medium shadow-xs font-medium leading-5 rounded-base text-sm px-4 py-2.5 focus:outline-none">
        {{__('admin.edit_blog',['action'=>__('admin.add')])}} </a>
  </div>
  <x-admin.breadcrumb />
      ...
      <thead class="text-sm text-body bg-neutral-secondary-medium border-b border-t border-default-medium">
        <tr>
          ...
          <th scope="col" class="px-6 py-3 font-medium">
           {{__('admin.edit_blog',['action' => __('admin.name')])}}
          </th>
          <th scope="col" class="px-6 py-3 font-medium">
            {{__('admin.path')}}
          </th>
